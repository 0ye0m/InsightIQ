 ...
 * Generate business-focused insights from the dataset.
 * @param {object} stats  output of computeStatistics (with rows, headers, columnProfiles)
 * @param {object} plan   optional LLM analysis plan from planner.js — overrides the
 *                         static primaryMeasure / primaryDimension / primaryTimeDimension
 *                         selection in the OLAP engine
 * @returns {object} { insights, anomalies, kpis, predictions, executiveSummary, olap }
 */
export function generateInsights(stats, plan = null) {
  const { rows, headers, columnProfiles, _numericValues } = stats;

  // Run the OLAP analysis — pass the plan so the OLAP engine uses the
  // LLM planner's choices for primaryMeasure / primaryDimension instead
  // of the static keyword rules.
  const olap = runOlapAnalysis(rows || [], headers || [], columnProfiles || {}, stats, plan);
  ...
  return {
    ...
    olap,
    classified: olap.classified,
    // LLM planner suggestions (domain + suggested KPIs/charts/insights)
    planSuggestions: olap.planSuggestions,
    // Legacy fields (still used by some UI components)
    dataHealth: buildDataHealthNote(stats),
  };
}
