...
/**
 * Pluralize a column name when used in a sentence like "top 3 products".
 * Handles common cases:
 *   • "product"     → "products"
 *   • "Pclass"      → "Pclasses" (already ends in 's' but is a single word)
 *   • "country"     → "countries"
 *   • "Status"      → "Statuses"
 * Note: column names that already end in 's' (like "Pclass", "Status")
 * still need "es" → "Pclasses", "Statuses".
 */
...
export function classifyColumns(rows, headers, columnProfiles) {
  ...
  // Order matters: earlier checks short-circuit later ones.
  //   1. date type           → timeDimension
  //   2. year-as-int         → timeDimension (NOT a measure)
  //   3. explicit ID column  → identifier (NOT a measure)
  //   4. high-cardinality numeric with ID-like name → identifier
  //   5. low-cardinality int  → dimension (encoded category)
  //   6. numeric, name screams measure → measure
  //   7. numeric, otherwise   → measure (default)
  //   8. boolean             → dimension
  //   9. high-cardinality text → identifier
  //  10. category/text        → dimension (with geo sub-classification)
  ...
  for (const h of headers) {
    ...
    // 1. DATE → time dimension (always)
    if (type === 'date') {
      result.timeDimensions.push({ column: h, ...stat });
      continue;
    }

    // 2. YEAR-AS-INT → time dimension
    // A column of integers where 80%+ of values are 4-digit years (1900-2099)
    // is a year column, NOT a measure. Aggregating years (sum, mean) is
    // meaningless and outlier detection on years produces false positives.
    if (type === 'number' || type === 'integer') {
      const yearMatches = rows
        .map((r) => r[h])
        .filter((v) => v !== null && v !== undefined && v !== '')
        .filter((v) => /^\d{4}$/.test(String(v)) && Number(v) >= 1900 && Number(v) <= 2099);
      const yearRatio = stat.nonNullCount ? yearMatches.length / stat.nonNullCount : 0;
      const looksLikeYear = yearRatio >= 0.8 && stat.uniqueCount > 1 && stat.uniqueCount < 200;
      if (looksLikeYear) {
        result.timeDimensions.push({ column: h, ...stat, type: 'year' });
        continue;
      }
    }

    // 3. EXPLICIT ID COLUMN → identifier
    // Test for common ID-column name patterns. We test against:
    //   - bare "id"               (column literally named "id")
    //   - "xxx_id"               (snake_case: passenger_id, order_id)
    //   - "xxxId"                 (camelCase: passengerId, orderId)
    //   - "xxx-id"                (kebab-case: passenger-id)
    //   - explicit ID keywords    (uuid, guid, code, key, sku, isbn, etc.)
    // The previous regex used \b which only matches at word boundaries —
    // "passengerid" has no word boundary before "id", so it slipped through
    // and got classified as a measure. This fix uses substring matching.
    if (type === 'number' || type === 'integer' || type === 'identifier') {
      const explicitId = isExplicitIdentifier(nameLower, stat);
      if (explicitId) {
        result.identifiers.push({ column: h, ...stat });
        continue;
      }
    ...
    // 4. NUMERIC COLUMN — measure vs dimension
    if (type === 'number' || type === 'integer') {
      // Low-cardinality numeric → categorical dimension encoded as int.
      // Rule: if the column has ≤ 15 distinct values AND those values
      // represent a small set of categories (like Survived 0/1, Pclass
      // 1/2/3, status codes), treat it as a dimension, NOT a measure.
      // We require rows.length > 5 to avoid misclassifying tiny samples.
      // The name must NOT match a measure keyword (e.g. "count" of 3 is
      // still a measure, not a category).
      const lowCardinalityThreshold = Math.max(15, Math.ceil(rows.length * 0.05));
      if (stat.uniqueCount <= lowCardinalityThreshold && rows.length > 5) {
        const measureName = MEASURE_KEYWORDS.some((kw) => nameLower.includes(kw));
        if (!measureName) {
          // Extra check: if values look like 0/1 booleans or small enum,
          // definitely a dimension
          const distinctVals = Array.from(new Set(rows.map((r) => r[h]).filter((v) => v !== null && v !== undefined && v !== '')));
          const looksLikeEnum = distinctVals.every((v) => Number.isInteger(Number(v))) && distinctVals.length <= 15;
          if (looksLikeEnum) {
            result.dimensions.push({ column: h, type: 'category', ...stat, encodedAs: 'integer' });
            continue;
          }
        }
      }
    ...
    // High-cardinality free-text columns (uniqueRatio near 1.0 with many
    // distinct values) are usually identifiers — UUIDs, emails, names.
    if (stat.uniqueRatio >= 0.95 && rows.length >= 20 && stat.uniqueCount > 20) {
      const measureName = MEASURE_KEYWORDS.some((kw) => nameLower.includes(kw));
      if (!measureName) {
        result.identifiers.push({ column: h, ...stat });
...
/**
 * Test whether a column name + cardinality pattern indicates an identifier
 * (ID/key/code column) rather than a measure.
 *
 * We use substring matching (not \b word boundaries) because real-world
 * column names come in many casing styles:
 *   • "id", "ID"               — bare
 *   • "passenger_id"           — snake_case
 *   • "PassengerId", "orderId" — camelCase  ← the bug we're fixing
 *   • "passenger-id"           — kebab-case
 *   • "orderNumber"            — also an identifier
 */
function isExplicitIdentifier(nameLower, stat) {
  // Bare "id" exactly
  if (nameLower === 'id') return true;

  // Ends with "_id", "-id", or "Id" (camelCase)
  if (/_id$|-id$/.test(nameLower)) return true;
  if (/id$/.test(nameLower) && stat.uniqueRatio >= 0.9) return true;

  // Starts with common ID prefixes
  if (/^(id|uuid|guid|sku|isbn|asin|serial)/.test(nameLower)) return true;

  // Contains explicit ID keywords as a whole token (via snake/kebab/space)
  const tokens = nameLower.split(/[_\-\s.]+/);
  const idTokens = ['id', 'uuid', 'guid', 'code', 'key', 'sku', 'isbn', 'asin', 'serial', 'hash', 'token', 'identifier'];
  if (tokens.some((t) => idTokens.includes(t))) return true;

  // CamelCase ID suffix: ends with "Id" (case-insensitive) AND high cardinality
  if (/[a-z]id$/.test(nameLower) && stat.uniqueRatio >= 0.9 && stat.uniqueCount >= 20) return true;

  return false;
}

/**
 * Pick the "primary" measure for default views. Prefers measures whose
 * name matches a known measure keyword (revenue, sales, amount, etc.).
 * Falls back to the first measure if none match by name.
 */
function pickPrimaryMeasure(measures) {
  if (!measures || !measures.length) return null;
  // Score each measure by name match
  const scored = measures.map((m) => {
    const nameLower = m.column.toLowerCase();
    let score = 0;
    for (const kw of MEASURE_KEYWORDS) {
      if (nameLower.includes(kw)) score += 1;
    }
    // Bonus for high sum (more "impactful" measure)
    if (typeof m.sum === 'number' && m.sum > 0) score += 0.5;
    return { column: m.column, score };
  });
  scored.sort((a, b) => b.score - a.score);
  return scored[0]?.column || measures[0].column;
}

/**
 * Pick the "primary" dimension for default Top-N / Pareto views.
 * Prefers:
 *   1. Non-boolean dimensions (booleans like Survived 0/1 make boring
 *      Top-N charts — just 2 bars)
 *   2. Dimensions with 3-20 unique values (more interesting than 2, not
 *      so many that the chart becomes unreadable)
 *   3. Geographic dimensions (often the most analytically useful)
 * Falls back to the first dimension if none qualify.
 */
function pickPrimaryDimension(dimensions) {
  if (!dimensions || !dimensions.length) return null;
  const scored = dimensions.map((d) => {
    let score = 0;
    // Penalize booleans heavily
    if (d.type === 'boolean') score -= 10;
    // Reward 3-20 unique values
    if (d.uniqueCount >= 3 && d.uniqueCount <= 20) score += 5;
    else if (d.uniqueCount >= 2 && d.uniqueCount <= 50) score += 2;
    // Reward geographic dimensions
    if (d.column && GEOGRAPHIC_KEYWORDS.some((kw) => d.column.toLowerCase().includes(kw))) score += 3;
    // Reward category type over integer-encoded
    if (d.type === 'category') score += 1;
    return { column: d.column, score, uniqueCount: d.uniqueCount, type: d.type };
  });
  scored.sort((a, b) => b.score - a.score);
  return scored[0]?.column || dimensions[0].column;
}
 ...
 * @param {object} columnProfiles
 * @param {object} stats  from computeStatistics
 * @param {object} plan   optional LLM analysis plan from planner.js
 * @returns {object} { classified, olap, businessSummary, businessInsights, businessAnomalies }
 */
export function runOlapAnalysis(rows, headers, columnProfiles, stats, plan = null) {
  // 1. Classify columns
  const classified = classifyColumns(rows, headers, columnProfiles);

  // 2. Pick the primary measure and primary dimension for default views.
  //
  // NEW: If an LLM analysis plan was provided (from src/engine/planner.js),
  // use the plan's choices for primaryMeasure / primaryDimension /
  // primaryTimeDimension — but ONLY if those columns actually exist in
  // the dataset AND are classified appropriately (e.g. the plan's
  // primaryMeasure must be in classified.measures).
  //
  // Otherwise, fall back to the static keyword-based pickers.
  let primaryMeasure = null;
  let primaryDimension = null;
  let primaryTimeDimension = null;

  if (plan && plan.primaryMeasure) {
    // Validate the plan's choices against the actual classification
    const planMeasure = classified.measures.find((m) => m.column === plan.primaryMeasure);
    if (planMeasure) primaryMeasure = plan.primaryMeasure;
    const planDim = [...classified.dimensions, ...classified.geoDimensions].find((d) => d.column === plan.primaryDimension);
    if (planDim) primaryDimension = plan.primaryDimension;
    if (plan.primaryTimeDimension) {
      const planTime = classified.timeDimensions.find((t) => t.column === plan.primaryTimeDimension);
      if (planTime) primaryTimeDimension = plan.primaryTimeDimension;
    }
  }

  // Fall back to the static pickers if the plan didn't provide valid choices
  if (!primaryMeasure) primaryMeasure = pickPrimaryMeasure(classified.measures) || null;
  if (!primaryDimension) primaryDimension = pickPrimaryDimension([...classified.dimensions, ...classified.geoDimensions]) || null;
  if (!primaryTimeDimension) primaryTimeDimension = classified.timeDimensions[0]?.column || null;
  ...
  // Anomalies from stats (IQR outliers) — re-surface as business anomalies.
  // SKIP columns that are identifiers or time dimensions — outlier detection
  // on ID columns (PassengerId, OrderId) or year columns (release_year)
  // produces false positives that confuse users.
  if (stats && stats.columnStats) {
    const skipCols = new Set([
      ...classified.identifiers.map((i) => i.column),
      ...classified.timeDimensions.map((t) => t.column),
    ]);
    for (const h of stats.numericColumns || []) {
      if (skipCols.has(h)) continue;  // don't run outlier detection on IDs/years
      const c = stats.columnStats[h];
      if (c && c.outliers > 0) {
        // Find actual outlier records
        const vals = numericValues(rows, h);
  ...
  if (primaryMeasure && primaryDimension) {
    ...
    if (top && top.length > 0) {
      ...
      if (top.length >= 3) {
        const top3Share = top.slice(0, 3).reduce((s, x) => s + x.share, 0);
        businessInsights.push(
          `The top 3 ${plural(primaryDimension)} account for ${pct(top3Share)} of all ${primaryMeasure}, indicating ${top3Share > 60 ? 'high' : 'moderate'} concentration.`
        );
      }
    }
  }
  ...
  if (primaryMeasure && primaryDimension) {
    ...
    if (pareto && pareto.paretoCount > 0) {
      businessInsights.push(
        `Pareto analysis: the top ${pareto.paretoCount} ${plural(primaryDimension)} (${pct(pareto.concentrationIndex)} of all) drive ${pct(pareto.cumulativeAt80)} of total ${primaryMeasure}.`
      );
    }
  }
  ...
  // 5. Build business summary (the executive narrative)
  let businessSummary = buildBusinessSummary(rows, classified, olap, primaryMeasure, primaryDimension, primaryTimeDimension);

  // If the LLM planner identified a domain, prepend it to the summary
  // so the user sees "Domain: E-commerce sales — ..." at the start.
  if (plan && plan.domain && plan.domain !== 'Generic dataset') {
    const domainPrefix = plan.domainDescription
      ? `Domain: ${plan.domain} — ${plan.domainDescription}`
      : `Domain: ${plan.domain}`;
    businessSummary = `${domainPrefix}. ${businessSummary}`;
  }

  // 6. Generate business KPIs
  const kpis = generateBusinessKpis(rows, classified, olap);

  // 6b. If the LLM planner suggested specific KPIs / charts / insights,
  // expose them so the UI can render an "AI analysis plan" panel.
  const planSuggestions = plan && plan.source === 'llm' ? {
    domain: plan.domain,
    domainDescription: plan.domainDescription,
    suggestedKpis: plan.kpis || [],
    suggestedCharts: plan.charts || [],
    suggestedInsights: plan.insights || [],
...
function buildBusinessSummary(rows, classified, olap, primaryMeasure, primaryDimension, primaryTimeDimension) {
  ...
  // ── CONTEXT: what IS this dataset? ─────────────────────────────────
  // Lead with the dataset's shape: records × columns, and what kinds of
  // data it holds.
  const measureNames = classified.measures.map((m) => m.column);
  const dimensionNames = [...classified.dimensions, ...classified.geoDimensions].map((d) => d.column);
  const timeNames = classified.timeDimensions.map((t) => t.column);

  parts.push(`This dataset contains ${rows.length.toLocaleString()} records across ${classified.measures.length + dimensionNames.length + timeNames.length + classified.identifiers.length} columns`);

  // Describe column composition
  const composition = [];
  if (measureNames.length) composition.push(`${measureNames.length} measure${measureNames.length === 1 ? '' : 's'} (${measureNames.slice(0, 3).join(', ')}${measureNames.length > 3 ? '...' : ''})`);
  if (dimensionNames.length) composition.push(`${dimensionNames.length} dimension${dimensionNames.length === 1 ? '' : 's'} (${dimensionNames.slice(0, 3).join(', ')}${dimensionNames.length > 3 ? '...' : ''})`);
  if (timeNames.length) composition.push(`${timeNames.length} time field${timeNames.length === 1 ? '' : 's'} (${timeNames.join(', ')})`);
  if (classified.identifiers.length) composition.push(`${classified.identifiers.length} identifier${classified.identifiers.length === 1 ? '' : 's'}`);
  if (composition.length) {
    parts.push(`comprising ${composition.join(', ')}`);
  }

  // ── HEADLINE INSIGHT: the single most important fact ──────────────
  // Prefer: top performer by primary measure (if a real measure exists).
  // Only mention if primaryMeasure is a genuine business measure (not an
  // ID column masquerading as a measure).
  if (primaryMeasure && primaryDimension) {
    const primaryMeasureObj = classified.measures.find((m) => m.column === primaryMeasure);
    if (primaryMeasureObj && typeof primaryMeasureObj.sum === 'number' && primaryMeasureObj.sum !== 0) {
      const top = olap.topN[`${primaryMeasure}__${primaryDimension}__sum`];
      if (top && top.length > 0) {
        parts.push(`Total ${primaryMeasure} is ${fmt(primaryMeasureObj.sum)}`);
        parts.push(`with "${top[0].value}" as the top ${primaryDimension} (${pct(top[0].share)} share)`);
      }
    }
  } else if (primaryMeasure && classified.measures.length > 0) {
    // No dimension — just state the measure total
    const m = classified.measures.find((x) => x.column === primaryMeasure) || classified.measures[0];
    if (typeof m.sum === 'number') {
      parts.push(`Total ${m.column} is ${fmt(m.sum)} (avg ${fmt(m.mean)})`);
    }
  }

  // ── TIME TREND (if a time dimension exists) ───────────────────────
  if (primaryMeasure && primaryTimeDimension) {
    const pop = olap.periodOverPeriod[`${primaryMeasure}__${primaryTimeDimension}__month`];
    if (pop && pop.length >= 2) {
      const last = pop[pop.length - 1];
      ...
      const totalGrowth = first.value !== 0 ? +(((last.value - first.value) / Math.abs(first.value)) * 100).toFixed(2) : null;
      if (totalGrowth !== null) {
        parts.push(`${primaryMeasure} ${totalGrowth >= 0 ? 'grew' : 'declined'} ${Math.abs(totalGrowth)}% from ${first.period} to ${last.period}`);
      }
    }
  }
  ...
  if (primaryMeasure && primaryDimension) {
    const pareto = olap.paretos[`${primaryMeasure}__${primaryDimension}`];
    if (pareto && pareto.paretoCount > 0 && pareto.paretoCount < pareto.totalContributors) {
      parts.push(`top ${pareto.paretoCount} ${plural(primaryDimension)} drive ${pct(pareto.cumulativeAt80)} of total ${primaryMeasure}`);
    }
  }
