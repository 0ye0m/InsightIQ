...
// InsightIQ — useAnalysis orchestration hook
//
// Pipeline: parse → plan (LLM) → clean → compute statistics → compute quality →
//           generate deterministic insights → build chart bundle →
//           index the RAG store → request a narrative from the engine.
//
// Every step before "narrative" is deterministic and offline-safe.
...
import { RetrievalPipeline } from '../rag/retrieval.js';
import { generateNarrative } from '../engine/generate.js';
import { generateAnalysisPlan } from '../engine/planner.js';
import { addHistoryEntry } from '../lib/history.js';
...
export function useAnalysis() {
  ...
  const run = useCallback(async (overrideParsed) => {
    ...
    try {
      // 0. LLM ANALYSIS PLANNER — runs BEFORE the deterministic analysis.
      // Looks at headers + a sample of rows and asks the generation engine:
      //   "What domain is this? What KPIs should we compute? What charts
      //    to prioritize? What insights to surface?"
      // The plan is then passed to the OLAP engine so it uses the LLM's
      // choices for primaryMeasure / primaryDimension / primaryTimeDimension
      // instead of the static keyword rules.
      // If the LLM is unavailable, the planner falls back to a deterministic
      // plan based on column-name keywords.
      let plan = null;
      setLoadingMsg('Planning analysis (LLM)…');
      try {
        plan = await generateAnalysisPlan(source);
      } catch (e) {
        console.warn('Analysis planner failed, using defaults:', e.message);
      }
      ...
      // 4. Insights (deterministic) — pass the LLM plan so the OLAP engine
      // uses the planner's primaryMeasure / primaryDimension choices.
      setLoadingMsg('Generating insights…');
      const insights = generateInsights(stats, plan);
      ...
      const assembled = {
        ...
        insights,
        chartBundle,
        plan,  // ← expose the LLM plan to the UI
      };
